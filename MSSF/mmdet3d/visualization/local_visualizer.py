...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def set_bev_image(self,
        ...
        if bev_image is None:
            bev_image = np.zeros((bev_shape, bev_shape, 4), np.uint8)
            #bev_image = 255 * np.ones((bev_shape, bev_shape, 4), np.uint8)
            #bev_image[:, :, :3] = 127
        ...
        self.ax_save.plot(
            ...
            markeredgecolor='red')
    @master_only
    def draw_bev_points(self, 
                        points: Union[np.ndarray, Tensor],
                        scale: float = 1,
                        colors: Union[str, tuple, List[str], List[tuple], List[float]] = 'g',
                        marker: Optional[str] = None,
                        sizes: Optional[Union[np.ndarray, Tensor]] = None,
                        cmap: Optional[str] = None) -> MMENGINE_Visualizer:
        """Draw points to BEV.

        Args:
            points (np.ndarray or Tensor): (N, 3+C)
        """
        bev_points = points[:, :2] * scale
        bev_points[:, 1] += self.width / 2
        bev_points[:, [0, 1]] = bev_points[:, [1, 0]]
        bev_points[:, 0] = self.width - bev_points[:, 0]
        return self.draw_points(bev_points, colors, marker, sizes, cmap)
