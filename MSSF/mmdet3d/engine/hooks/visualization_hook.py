...
class Det3DVisualizationHook(Hook):
    ...
    def __init__(self,
        ...
        self.output_detsample_dir = output_detsample_dir
    ...
    def after_test_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        Args:
            ...
            outputs (Sequence[:obj:`DetDataSample`]): A batch of data samples
                that contain annotations and predictions.
        """
        if self.output_detsample_dir is not None:
            outputs_save = []
            for i in range(len(outputs)):
                output_cpu = outputs[i].cpu()
                # TODO: 漏网之鱼
                if 'img_std' in output_cpu.all_keys():
                    output_cpu.pop('img_std')
                    output_cpu.pop('img_mean')
                if 'centroids' in output_cpu.keys():
                    output_cpu.pop('centroids')
                if 'seg_preds' in output_cpu.keys():
                    output_cpu.pop('seg_preds')
                output_cpu.pred_instances_3d.bboxes_3d = output_cpu.pred_instances_3d.bboxes_3d.cpu()
                output_cpu.gt_instances_3d.bboxes_3d = output_cpu.eval_ann_info['gt_bboxes_3d']
                output_cpu.gt_instances_3d.labels_3d = output_cpu.eval_ann_info['gt_bboxes_labels']
                outputs_save.append(output_cpu)
                        
            dump(outputs_save, osp.join(self.output_detsample_dir, f'{batch_idx}.pkl'))
