...
def clean_data(gt_anno, dt_anno, current_class, difficulty, roi_clean=False, roi_out=False):  # per frame
    #valid_class_names = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    valid_class_names = ['car_motion', 'pedestrian_motion', 'cyclist_motion', 'car_static', 'pedestrian_static', 'cyclist_static']
    ...
    for i in range(num_gt):  # per frame
        ...
        if roi_out:
            x, y, z = gt_anno['location'][i]
            if x >= left and x <= right and z <= max_distance:
                ignore = True  
    ...
    for i in range(num_dt):
        ...
        if height < min_instance_height[difficulty]:
            ...
            ignored_dt.append(1)
        elif (x >= left and x <= right and z <= max_distance) and roi_out:
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
        else:
...
def _prepare_data(gt_annotations, dt_annotations, current_class, difficulty, custom_method=0):
    ...
    for i in range(len(gt_annotations)):
        ...
        if custom_method == 1:
            rets = clean_data(gt_annotations[i], dt_annotations[i], current_class, difficulty, roi_out=True)
...
def kitti_eval(gt_annotations, dt_annotations, current_classes, eval_types=['bbox', 'bev', '3d'],pr_detail_dict=None, custom_method=0):
    ...
    elif custom_method == 1:
        print('Evaluating kitti by ROI out')
    ...
    overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5],
                            ...
                            [0.7, 0.5, 0.5, 0.7, 0.5, 0.5]])
    overlap_0_5 = np.array([[0.7, 0.50, 0.50, 0.7, 0.50, 0.25],  # image
        ...
        for i in range(1, 2):  # min_overlaps.shape[0]):
            ...
            if i == 1:
                ...
                ret_dict['%s_bev_all_r40_strict' % class_to_name[curcls]] = mAPbev_R40[
                    j, 0, 0]  # get j class, difficulty, first min_overlap
    if custom_method == 0:
        area_name = 'Entire annotated area:'
    elif custom_method == 1:
        area_name = 'Out Driving corridor area:'
    elif custom_method == 3:
        area_name = 'Driving corridor area:'
    result_str = "Results: \n" \
                    +f"{area_name} \n" \
                    +f"Car motion: {ret_dict['Car_motion_3d_all']} \n" \
                    +f"Pedestrian motion: {ret_dict['Pedestrian_motion_3d_all']} \n" \
                    +f"Cyclist motion: {ret_dict['Cyclist_motion_3d_all']} \n" \
                    +f"mAP: {(ret_dict['Car_motion_3d_all'] + ret_dict['Pedestrian_motion_3d_all'] + ret_dict['Cyclist_motion_3d_all']) / 3} \n" \
                    +"\n" \
                    +f"Car static: {ret_dict['Car_static_3d_all']} \n" \
                    +f"Pedestrian static: {ret_dict['Pedestrian_static_3d_all']} \n" \
                    +f"Cyclist static: {ret_dict['Cyclist_static_3d_all']} \n" \
                    +f"mAP: {(ret_dict['Car_static_3d_all'] + ret_dict['Pedestrian_static_3d_all'] + ret_dict['Cyclist_static_3d_all']) / 3} \n" \
                    +"\n"
    ...
    #                 +f"mAP: {(ret_dict['Car_static_3d_all'] + ret_dict['Pedestrian_static_3d_all'] + ret_dict['Cyclist_static_3d_all']) / 3} \n" \
    #                 +"\n" \
    #                 +f"Car motion strict: {ret_dict['Car_motion_3d_all_strict']} \n" \
    #                 +f"Pedestrian motion strict: {ret_dict['Pedestrian_motion_3d_all_strict']} \n" \
    #                 +f"Cyclist motion strict: {ret_dict['Cyclist_motion_3d_all_strict']} \n" \
    #                 +f"mAP strict: {(ret_dict['Car_motion_3d_all_strict'] + ret_dict['Pedestrian_motion_3d_all_strict'] + ret_dict['Cyclist_motion_3d_all_strict']) / 3} \n" \
    #                 +f"Car static strict: {ret_dict['Car_static_3d_all_strict']} \n" \
    #                 +f"Pedestrian static strict: {ret_dict['Pedestrian_static_3d_all_strict']} \n" \
    #                 +f"Cyclist static strict: {ret_dict['Cyclist_static_3d_all_strict']} \n" \
    #                 +f"mAP strict: {(ret_dict['Car_static_3d_all_strict'] + ret_dict['Pedestrian_static_3d_all_strict'] + ret_dict['Cyclist_static_3d_all_strict']) / 3} \n" \
    #                 +"\n" \
    #                 +f"Car motion R40: {ret_dict['Car_motion_3d_all_r40']} \n" \
    #                 +f"Pedestrian motion R40: {ret_dict['Pedestrian_motion_3d_all_r40']} \n" \
    #                 +f"Cyclist motion R40: {ret_dict['Cyclist_motion_3d_all_r40']} \n" \
    #                 +f"mAP R40: {(ret_dict['Car_motion_3d_all_r40'] + ret_dict['Pedestrian_motion_3d_all_r40'] + ret_dict['Cyclist_motion_3d_all_r40']) / 3} \n" \
    #                 +f"Car static R40: {ret_dict['Car_static_3d_all_r40']} \n" \
    #                 +f"Pedestrian R40: {ret_dict['Pedestrian_static_3d_all_r40']} \n" \
    #                 +f"Cyclist R40: {ret_dict['Cyclist_static_3d_all_r40']} \n" \
    #                 +f"mAP R40: {(ret_dict['Car_static_3d_all_r40'] + ret_dict['Pedestrian_static_3d_all_r40'] + ret_dict['Cyclist_static_3d_all_r40']) / 3} \n" \
    #                 +"\n" \
    #                 +f"Car motion strict R40: {ret_dict['Car_motion_3d_all_r40_strict']} \n" \
    #                 +f"Pedestrian motion strict R40: {ret_dict['Pedestrian_motion_3d_all_r40_strict']} \n" \
    #                 +f"Cyclist motion strict R40: {ret_dict['Cyclist_motion_3d_all_r40_strict']} \n" \
    #                 +f"mAP strict R40: {(ret_dict['Car_motion_3d_all_r40_strict'] + ret_dict['Pedestrian_motion_3d_all_r40_strict'] + ret_dict['Cyclist_motion_3d_all_r40_strict']) / 3} \n" \
    #                 +f"Car static strict R40: {ret_dict['Car_static_3d_all_r40_strict']} \n" \
    #                 +f"Pedestrian static strict R40: {ret_dict['Pedestrian_static_3d_all_r40_strict']} \n" \
    #                 +f"Cyclist static strict R40: {ret_dict['Cyclist_static_3d_all_r40_strict']} \n" \
    #                 +f"mAP strict R40: {(ret_dict['Car_static_3d_all_r40_strict'] + ret_dict['Pedestrian_static_3d_all_r40_strict'] + ret_dict['Cyclist_static_3d_all_r40_strict']) / 3} \n" \
    #                 +"\n"
    ...
    if custom_method == 0:
        ...
        return result_str, {'roi_out': ret_dict}
    elif custom_method == 3:
