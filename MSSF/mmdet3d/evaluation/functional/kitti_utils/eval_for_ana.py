...
def compute_statistics_jit(overlaps,
    ...
    fn_idx = np.zeros((gt_size, ), dtype=np.int32)
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], fn_idx
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        gt_datas_3d = np.concatenate(
            [gt_annos[i]['dimensions'], gt_annos[i]['location'], gt_annos[i]['rotation_y'][..., np.newaxis], gt_annos[i]['score'][..., np.newaxis]], 1)
        gt_datas_list.append(gt_datas)
        dt_datas_list.append(dt_datas)
        gt_datas_3d_list.append(gt_datas_3d)
    total_dc_num = np.stack(total_dc_num, axis=0)
    return (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets, dontcares,
            total_dc_num, total_num_valid_gt, gt_datas_3d_list)
    ...
    for m, current_class in enumerate(current_classes):
        for idx_l, difficulty in enumerate(difficultys):
            ...
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt, gt_datas_3d_list) = rets
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(
                        ...
                        thresh=0.0,
                        compute_fp=False)
                    tp, fp, fn, similarity, thresholds, fn_idx = rets
                    fn_idx = np.where(fn_idx==1)[0]
                    fn_bboxes_3d.append(gt_datas_3d_list[i][fn_idx])
                    fn_bboxes.append(gt_datas_list[i][fn_idx, :4])
                    thresholdss += thresholds.tolist()

                if difficulty == difficultys[-1] and min_overlap == min_overlaps[:, metric, m][-1]:
                    import os
                    class_to_name = {
                            0: 'Car',
                            1: 'Pedestrian',
                    ...
                    save_path = f'det_results/eval_ana/label_2_fn_{class_to_name[current_class]}_75'
                    if not os.path.exists(save_path):
                        os.mkdir(save_path)
                    total_fn = 0
                    for i, (fn_3d, fn_2d) in enumerate(zip(fn_bboxes_3d, fn_bboxes)):
                        total_fn += fn_3d.shape[0]
                        with open(os.path.join(save_path, f'{i:05d}.txt'), 'w') as f:
                            lines = []
                            for j in range(fn_3d.shape[0]):
                                line = f'{class_to_name[current_class]} -1 -1 0 ' + ' '.join(list(map(str, list(fn_2d[j])))) + ' ' + ' '.join(list(map(str, list(fn_3d[j])))) + '\n'
                                lines.append(line)
                            f.writelines(lines)
                    print(f'Total FNs of class {class_to_name[current_class]}: {total_fn}')
