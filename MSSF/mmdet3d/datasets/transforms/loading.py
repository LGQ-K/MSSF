...
class LoadPrecomputeImgFeats(BaseTransform):
    """LoadPrecomputeImgFeats
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        frame_id = filename.split('/')[-1].split('.')[0]
        with open(os.path.join(self.data_path, frame_id), 'rb') as f:
            img_feats = pickle.load(f)
        results['img_feats'] = img_feats
        return results
...
@TRANSFORMS.register_module()
class LoadEvalAnnotations3D(LoadAnnotations):
    """Load Annotations3D.
