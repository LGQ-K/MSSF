...
@TRANSFORMS.register_module()
class EmptyGTBoxesFilter(BaseTransform):
    """Filter Empty GT objects.
...
@TRANSFORMS.register_module()
class JitterCalibration(BaseTransform):
    def __init__(self, sigma_r, sigma_t):
    ...
    def perturb_T(self, T):
        """
        Perturb the extrinsic parameters T by adding small rotations and translations.

        Parameters:
        - T: Original extrinsic parameter matrix (4x4 numpy array).
        - sigma_r: Standard deviation for rotation perturbation (in radians).
        - sigma_t: Standard deviation for translation perturbation (in the same units as t).

        Returns:
        - T_perturbed: Perturbed extrinsic parameter matrix (4x4 numpy array).
        """
        ...
        # Generate small random rotation perturbation
        delta_rot_vec = np.random.normal(scale=sigma_r, size=3)  # Rotation vector perturbation

        # Convert rotation vector to rotation matrix using Rodrigues' formula
        delta_R = self.rotation_vector_to_matrix(delta_rot_vec)

        # Perturb rotation
        R_perturbed = delta_R @ R

        # Generate small random translation perturbation
        delta_t = np.random.normal(scale=sigma_t, size=3)

        # Perturb translation
        t_perturbed = t + delta_t

        # Reassemble perturbed T
    ...
    def rotation_vector_to_matrix(rot_vec):
        ...
        Parameters:
        - rot_vec: Rotation vector (numpy array of shape (3,))
    ...
    def transform(self, input_dict: dict) -> dict:
        assert np.abs(input_dict['cam2img']@input_dict['lidar2cam']-input_dict['lidar2img']).sum()<1e-6
        input_dict['lidar2img'] = input_dict['cam2img'] @ self.perturb_T(input_dict['lidar2cam'])
        return input_dict