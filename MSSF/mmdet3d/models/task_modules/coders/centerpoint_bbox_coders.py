...
class CenterPointBBoxCoder(BaseBBoxCoder):
    ...
    def __init__(self,
                 ...
                 max_num: int = 100,
                 task_max_num: Optional[List[int]] = None,
    ...
    def decode(self,
        ...
        if self.task_max_num is not None and task_id >= 0 and task_id < len(
                self.task_max_num):
            max_num = int(self.task_max_num[task_id])
        ...
        rot_sine = rot_sine.view(batch, max_num, 1)

        rot_cosine = self._transpose_and_gather_feat(rot_cosine, inds)
        rot_cosine = rot_cosine.view(batch, max_num, 1)
