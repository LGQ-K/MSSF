...
if IS_SPCONV2_AVAILABLE:
    from spconv.pytorch import SparseConvTensor, SparseSequential, SubMConv2d
else:
...
class SeparateHeadVoxelNeXt(BaseModule):
    """SeparateHeadVoxelNeXt for VoxelNeXt.
    ...
    def __init__(self,
        ...
        super(SeparateHeadVoxelNeXt, self).__init__(init_cfg=init_cfg)
        self.heads = heads
            ...
            for i in range(num_conv - 1):
                ...
                conv_layers.append(
                    make_sparse_convmodule(
                        c_in, head_conv, final_kernel, head, 1, padding=(final_kernel//2), conv_type='SubMConv2d', norm_cfg=dict(type='BN1d'), use_bias_before_norm=True
                    )
                )
                c_in = head_conv

            conv_layers.append(
                SubMConv2d(head_conv, classes, 1, bias=True, indice_key=head+'out'))
            conv_layers = nn.Sequential(*conv_layers)

            self.__setattr__(head, conv_layers)

        if init_cfg is None:
            self.init_cfg = dict(type='Kaiming', layer='SubMConv2d')
        for head in self.heads:
            if head == 'heatmap':
                self.__getattr__(head)[-1].bias.data.fill_(self.init_bias)
            else:
                for m in self.__getattr__(head).modules():
                    if isinstance(m, SubMConv2d):
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            ret_dict[head] = out.features

            debug = False
            if head == 'heatmap' and debug:
                from mmdet3d.visualization import Det3DLocalVisualizer
                from mmengine.visualization import Visualizer
                vis: Det3DLocalVisualizer = Visualizer.get_current_instance()
                for i in range(3):
                    vis.set_image(out.dense()[0].sigmoid()[i].T.flip(0).flip(1).cpu().numpy()*255)
                    vis.fig_save_canvas.print_figure(f'./middle_vis_2/hm_{i}.png')
        return ret_dict

def _draw_gaussian_to_heatmap_voxels(heatmap, distances, radius, k=1):
    diameter = 2 * radius + 1
    sigma = diameter / 6
...
class VoxelNeXtHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 gaussian_type: Optional[List[str]] = ['nearst', 'gt_center'],
        ...
        for num_cls in num_classes:
            ...
            separate_head.update(
                in_channels=in_channels, heads=heads, num_cls=num_cls, head_conv=share_conv_channel)
    ...
    def forward(self, feats: List[Tensor]) -> Tuple[List[Tensor]]:
        ...
        """
        assert len(feats) == 1
        spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels = self._get_voxel_infos(feats[0])
        voxel_infos = {
        ...
        return multi_apply(self.forward_single, feats), voxel_infos
    ...
    def get_targets(
        ...
        # return ret_dict
        spatial_shape = voxel_infos['spatial_shape']
        batch_index = voxel_infos['batch_index']
        voxel_indices = voxel_infos['voxel_indices']
        spatial_indices = voxel_infos['spatial_indices']
        num_voxels = voxel_infos['num_voxels']
        batch_voxel_infos = []
        for i in range(len(num_voxels)):
            batch_voxel_infos.append({
                'num_voxels': num_voxels[i],
                'spatial_indices': spatial_indices[i]
            })
        heatmaps, anno_boxes, inds, masks = multi_apply(
            self.get_targets_single, batch_gt_instances_3d, batch_voxel_infos)
        
        debug = False
        if debug:
            hm_vis = SparseConvTensor(
            features=heatmaps[0][0].permute(1,0),
            indices=voxel_indices[voxel_indices[:,0]==0],
            spatial_shape=spatial_shape,
            batch_size=1
            ).dense()
        # Transpose heatmaps
        heatmaps = list(map(list, zip(*heatmaps)))
        heatmaps = [torch.cat(hms_, dim=1).permute(1, 0) for hms_ in heatmaps]
    ...
    def get_targets_single(self,
                           gt_instances_3d: InstanceData, voxel_infos: dict) -> Tuple[Tensor]:
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            #      feature_map_size[0]))
            heatmap = gt_bboxes_3d.new_zeros((len(self.class_names[idx]), num_voxels))
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    distance = _distance(spatial_indices, center)
                    ind[k] = distance.argmin()
                    mask[k] = 1

                    if 'gt_center' in self.gaussian_type:
                        _draw_gaussian_to_heatmap_voxels(heatmap[cls_id], distance, radius)

                    if 'nearst' in self.gaussian_type:
                        _draw_gaussian_to_heatmap_voxels(heatmap[cls_id], _distance(spatial_indices, spatial_indices[ind[k]]), radius)
    ...
    def loss(self, pts_feats: List[Tensor],
        ...
        """
        outs, voxel_infos = self(pts_feats)
        batch_gt_instance_3d = []
        for data_sample in batch_data_samples:
            batch_gt_instance_3d.append(data_sample.gt_instances_3d)
        losses = self.loss_by_feat(outs, batch_gt_instance_3d, voxel_infos=voxel_infos)
        return losses
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        """
        voxel_infos = kwargs['voxel_infos']
        heatmaps, anno_boxes, inds, masks = self.get_targets(
            batch_gt_instances_3d, voxel_infos)
        loss_dict = dict()
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            pred = preds_dict[0]['anno_box']
            loss_bbox = self.loss_bbox(
                pred, mask, ind, target_box, voxel_infos['batch_index']
            ).sum()
    ...
    def predict(self,
        ...
        Returns:
            ...
            scores and labels.
        """
        preds_dict, voxel_infos = self(pts_feats)
        ...
        results_list = self.predict_by_feat(
            preds_dict, batch_input_metas, rescale=rescale, voxel_infos=voxel_infos, batch_size=batch_size, **kwargs)
        for b in range(batch_size):
            results_list[b].spatial_indices = voxel_infos['spatial_indices'][b][results_list[b].voxel_inds]
        return results_list
        ...
        Returns:
            ...
            Each item usually contains following keys.
                ...
                - bboxes_3d (:obj:`LiDARInstance3DBoxes`): Prediction
                  ...
                  velocity.
        """
        voxel_infos:dict = kwargs['voxel_infos']
        batch_size = kwargs['batch_size']
        voxel_infos.update(batch_size=batch_size)
        rets = []
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_corr_inds = [box['inds'] for box in temp]
        ...
        for i in range(num_samples):
            ...
            for k in rets[0][i].keys():
                if k == 'bboxes':
                    ...
                    inds = torch.cat([ret[i][k] for ret in rets])
            temp_instances.bboxes_3d = bboxes
            temp_instances.scores_3d = scores
            temp_instances.labels_3d = labels
            temp_instances.voxel_inds = inds
            ret_list.append(temp_instances)
        
        if 'points' in kwargs:
            batch_points = kwargs['points']
            for k, (dets, points) in enumerate(zip(ret_list, batch_points)):
                num_boxes = len(dets.bboxes_3d)
                box_idxs_of_pts = points_in_boxes_part(points[None,:,:3], dets.bboxes_3d.tensor[None, ...]).squeeze()
                selected = []
                for i in range(num_boxes):
                    points_num_in_box = (box_idxs_of_pts == i).sum()
                    if points_num_in_box > 0:
                        selected.append(i)
                if len(selected) == 0:
                    ret_list[k] = ret_list[k][:0]
                else:
                    ret_list[k] = ret_list[k][selected]

        if 'points_misc' in kwargs:
            #points_misc = kwargs['points_misc']['x_conv3']
            points_misc = kwargs['points_misc'][list(kwargs['points_misc'].keys())[-1]]
            for k, dets in enumerate(ret_list):
                num_boxes = len(dets.bboxes_3d)
                batch_mask = (points_misc['voxel_dict']['centroids'][:,0].int()==k)
                pred_pts_seg = points_misc['seg_results']['seg_preds'][batch_mask].sigmoid()
                voxel_centroids = points_misc['voxel_dict']['centroids'][batch_mask][:,1:4]
                box_idxs_of_pts = points_in_boxes_part(voxel_centroids[None,:,:3], dets.bboxes_3d.tensor[None, ...]).squeeze()
                for i in range(num_boxes):
                    mask = (box_idxs_of_pts == i)
                    if mask.sum() > 0:
                        max_score = pred_pts_seg[mask].max()
                        mean_score = pred_pts_seg[mask].mean()
                        ret_list[k].scores_3d[i] = 0.5 * ret_list[k].scores_3d[i] + 0.5 * max_score
        return ret_list

    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
                            batch_reg_preds, batch_cls_labels, batch_corr_inds, img_metas):
        ...
        for i, (box_preds, cls_preds, cls_labels, corr_inds) in enumerate(
                zip(batch_reg_preds, batch_cls_preds, batch_cls_labels, batch_corr_inds)):
            ...
            if top_scores.shape[0] != 0:
                if self.test_cfg['score_threshold'] > 0.0:
                    ...
                    corr_inds = corr_inds[top_scores_keep]
            ...
            selected_inds = corr_inds[selected]
