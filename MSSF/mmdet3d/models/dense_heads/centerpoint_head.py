...
class CenterHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 task_score_thresholds: Optional[List[float]] = None,
                 task_nms_thr: Optional[List[float]] = None,
                 task_score_gamma: Optional[List[float]] = None,
                 task_pre_max_size: Optional[List[int]] = None,
                 task_post_max_size: Optional[List[int]] = None,
                 task_small_object_max_dim: Optional[List[float]] = None,
                 task_small_nms_thr: Optional[List[float]] = None,
                 task_small_post_max_size: Optional[List[int]] = None,
                 task_min_keep_candidates: Optional[List[int]] = None,
                 task_min_radius: Optional[List[float]] = None,
                 task_gaussian_overlap: Optional[List[float]] = None,
        ...
        self.task_score_thresholds = task_score_thresholds
        self.task_nms_thr = task_nms_thr
        self.task_score_gamma = task_score_gamma
        self.task_pre_max_size = task_pre_max_size
        self.task_post_max_size = task_post_max_size
        self.task_small_object_max_dim = task_small_object_max_dim
        self.task_small_nms_thr = task_small_nms_thr
        self.task_small_post_max_size = task_small_post_max_size
        self.task_min_keep_candidates = task_min_keep_candidates
        self.task_min_radius = task_min_radius
        self.task_gaussian_overlap = task_gaussian_overlap
    ...
    def get_targets_single(self,
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            if (self.task_gaussian_overlap is not None
                    and idx < len(self.task_gaussian_overlap)):
                gaussian_overlap = float(self.task_gaussian_overlap[idx])
            min_radius = self.train_cfg['min_radius']
            if self.task_min_radius is not None and idx < len(self.task_min_radius):
                min_radius = float(self.task_min_radius[idx])
            ...
            #                                  dtype=torch.float32)
            anno_box = gt_bboxes_3d.new_zeros((max_objs, self.bbox_coder.code_size+1),
                                                dtype=torch.float32)
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    radius = int(max(min_radius, int(radius)))
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if (self.task_loss_weights is not None
                    and task_id < len(self.task_loss_weights)):
                task_loss_weight = float(self.task_loss_weights[task_id])
            # heatmap focal loss
            ...
            loss_dict[f'task{task_id}.loss_heatmap'] = (
                loss_heatmap * task_loss_weight)
            loss_dict[f'task{task_id}.loss_bbox'] = (
                loss_bbox * task_loss_weight)
        return loss_dict
    ...
    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            score_thr = self.test_cfg['score_threshold']
            if (self.task_score_thresholds is not None
                    and task_id < len(self.task_score_thresholds)):
                score_thr = float(self.task_score_thresholds[task_id])
            nms_thr = self.test_cfg['nms_thr']
            if self.task_nms_thr is not None and task_id < len(self.task_nms_thr):
                nms_thr = float(self.task_nms_thr[task_id])
            pre_max_size = self.test_cfg['pre_max_size']
            if (self.task_pre_max_size is not None
                    and task_id < len(self.task_pre_max_size)):
                pre_max_size = int(self.task_pre_max_size[task_id])
            post_max_size = self.test_cfg['post_max_size']
            if (self.task_post_max_size is not None
                    and task_id < len(self.task_post_max_size)):
                post_max_size = int(self.task_post_max_size[task_id])
            small_nms_thr = nms_thr
            if (self.task_small_nms_thr is not None
                    and task_id < len(self.task_small_nms_thr)):
                small_nms_thr = float(self.task_small_nms_thr[task_id])
            small_post_max_size = post_max_size
            if (self.task_small_post_max_size is not None
                    and task_id < len(self.task_small_post_max_size)):
                small_post_max_size = int(self.task_small_post_max_size[task_id])
            score_gamma = 1.0
            if (self.task_score_gamma is not None
                    and task_id < len(self.task_score_gamma)):
                score_gamma = float(self.task_score_gamma[task_id])
            min_keep_candidates = 0
            if (self.task_min_keep_candidates is not None
                    and task_id < len(self.task_min_keep_candidates)):
                min_keep_candidates = int(self.task_min_keep_candidates[task_id])
            ...
            if num_class_with_bg == 1:
                ...
                top_scores = cls_preds.squeeze(-1)
            if score_gamma != 1.0:
                # Size-aware score calibration:
                # only boost likely small objects to avoid hurting large ones.
                if (self.task_small_object_max_dim is not None
                        and task_id < len(self.task_small_object_max_dim)):
                    small_max_dim = float(self.task_small_object_max_dim[task_id])
                    box_xy_size = box_preds[:, 3:5]
                    box_max_dim = box_xy_size.max(dim=1).values
                    small_mask = box_max_dim <= small_max_dim
                    if small_mask.any():
                        boosted_scores = top_scores.clamp(min=1e-6).pow(score_gamma)
                        top_scores = torch.where(small_mask, boosted_scores,
                                                 top_scores)
                else:
                    top_scores = top_scores.clamp(min=1e-6).pow(score_gamma)

            top_scores_keep = torch.ones_like(top_scores, dtype=torch.bool)
            if score_thr > 0.0:
                thresh = torch.tensor(
                    [score_thr],
                    device=cls_preds.device).type_as(cls_preds)
                top_scores_keep = top_scores >= thresh
                # Small-object recall guard:
                # keep at least K candidates for selected tasks.
                if min_keep_candidates > 0 and top_scores.shape[0] > min_keep_candidates:
                    num_kept = int(top_scores_keep.sum().item())
                    if num_kept < min_keep_candidates:
                        topk_idx = torch.topk(
                            top_scores, k=min_keep_candidates).indices
                        top_scores_keep[topk_idx] = True
            ...
            if top_scores.shape[0] != 0:
                ...
                # stage-1: conservative task-level NMS
                selected = nms_bev(
                    boxes_for_nms,
                    top_scores,
                    thresh=nms_thr,
                ...
                # stage-2: small-object recovery NMS
                if (self.task_small_object_max_dim is not None
                        and task_id < len(self.task_small_object_max_dim)):
                    small_max_dim = float(self.task_small_object_max_dim[task_id])
                    box_xy_size = box_preds[:, 3:5]
                    small_mask = box_xy_size.max(dim=1).values <= small_max_dim
                    if small_mask.any():
                        small_indices = small_mask.nonzero(as_tuple=False).squeeze(-1)
                        small_boxes_for_nms = boxes_for_nms[small_indices]
                        small_scores = top_scores[small_indices]
                        if small_scores.shape[0] > 0:
                            selected_small_rel = nms_bev(
                                small_boxes_for_nms,
                                small_scores,
                                thresh=small_nms_thr,
                                pre_max_size=pre_max_size,
                                post_max_size=small_post_max_size)
                            selected_small = small_indices[selected_small_rel]
                            selected = torch.unique(
                                torch.cat([selected, selected_small], dim=0))
            else:
                selected = []
