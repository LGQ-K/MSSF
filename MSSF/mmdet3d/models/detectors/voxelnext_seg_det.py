...
class VoxelNeXtSegDet(MVXTwoStageDetector):
    ...
    def predict(self, batch_inputs_dict: Dict[str, Optional[Tensor]],
        ...
        # detsamples[0].seg_preds = dict()
        # detsamples[0].centroids = dict()
        # for loc, res in points_misc.items():
        #     pred_pts_seg = points_misc[loc]['seg_results']['seg_preds']
        #     voxel_centroids = points_misc[loc]['voxel_dict']['centroids'][:,1:4]
        #     detsamples[0].seg_preds[loc] = pred_pts_seg.squeeze()
        #     detsamples[0].centroids[loc] = voxel_centroids
        return detsamples
    ...
    def loss(self, batch_inputs_dict: Dict[List, torch.Tensor],
        ...
        """
        self.pts_middle_encoder.seg_branch.eval()
        batch_input_metas = [item.metainfo for item in batch_data_samples]
        img_feats, pts_feats, points_misc = self.extract_feat(batch_inputs_dict,
                                                 batch_input_metas)
        losses = dict()
