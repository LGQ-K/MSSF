...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if pts_middle_encoder:
            if seg_branch is not None:
                pts_middle_encoder.update(seg_branch=seg_branch)
    ...
    def extract_pts_feat(
        ...
        x = self.pts_middle_encoder(voxel_features, voxel_dict['coors'],
                                    batch_size, batch_inputs_dict=batch_inputs_dict, 
    ...
    def predict(self, batch_inputs_dict: Dict[str, Optional[Tensor]],
        ...
        if debug:
            ...
            import mmcv
            import os
            save_path = f'./middle_vis_2/{batch_input_metas[0]["img_path"][0].split(".")[0]}'
            if not os.path.exists(save_path):
                os.mkdir(save_path)
            vis: Det3DLocalVisualizer = Det3DLocalVisualizer()
            vis.set_bev_image(bev_shape=int(51.2*10))
            vis.draw_bev_points(batch_inputs_dict['points'][0].cpu().numpy(), scale=10, sizes=1)
            vis.draw_bev_bboxes(batch_data_samples[0].gt_instances_3d.bboxes_3d, edge_colors='orange', scale=10)
            vis.draw_bev_bboxes(results_list_3d[0].bboxes_3d, edge_colors='white', scale=10)
            pc_range = torch.tensor(self.train_cfg['pts']['point_cloud_range'])
            voxel_size = torch.tensor(self.train_cfg['pts']['voxel_size'])
            voxel_centers = results_list_3d[0].spatial_indices.float()
            voxel_centers[:,0] = results_list_3d[0].spatial_indices[:,0] * self.train_cfg['pts']['out_size_factor'] * voxel_size[0] + pc_range[0]
            voxel_centers[:,1] = results_list_3d[0].spatial_indices[:,1] * self.train_cfg['pts']['out_size_factor'] * voxel_size[1] + pc_range[1]
            vis.draw_bev_points(voxel_centers, scale=10, colors='r', sizes=1)
            vis.fig_save_canvas.print_figure(f'./middle_vis_2/{batch_input_metas[0]["img_path"][0].split(".")[0]}/result.png')
            im = mmcv.imread('./middle_vis_2/spatial_features.png')
            im[127-results_list_3d[0].spatial_indices[:, 0].cpu(), 128-results_list_3d[0].spatial_indices[:, 1].cpu(), 0]=0
            im[127-results_list_3d[0].spatial_indices[:, 0].cpu(), 128-results_list_3d[0].spatial_indices[:, 1].cpu(), 1]=0
            im[127-results_list_3d[0].spatial_indices[:, 0].cpu(), 128-results_list_3d[0].spatial_indices[:, 1].cpu(), 2]=255

            mmcv.imwrite(im, f'./middle_vis_2/{batch_input_metas[0]["img_path"][0].split(".")[0]}/spatial_features_pred.png')
        return detsamples
