...
from ..backbones.second_fuse_latency import SECONDFuseLatency

@MODELS.register_module()
class VoxelNetFuseLatency(SingleStage3DDetector):
    ...
    def __init__(self,
        ...
        self.img_feats_meter = AverageMeter()
        self.pts_feats_meter = AverageMeter()
        self.fusion_meter = AverageMeter()
        self.view_trans_meter = AverageMeter()

    def reset_all_meters(self):
        self.img_feats_meter.reset()
        self.pts_feats_meter.reset()
        self.fusion_meter.reset()
        self.view_trans_meter.reset()

    def print_latency(self):
        print(f'Image Feature Extraction Time: {self.img_feats_meter.avg}')
        print(f'View Transformation Time: {self.view_trans_meter.avg}')
        print(f'Fusion Time: {self.fusion_meter.avg}')
        print(f'Point Feature Extraction Time: {self.pts_feats_meter.avg}')

    def init_weights(self):
        """Initialize model weights."""
        super(VoxelNetFuseLatency, self).init_weights()
    ...
    def extract_img_feat(self, img: Tensor, input_metas: List[dict]) -> dict:
        ...
        if self.with_img_neck:
            ...
            self.img_feats_meter.update(end-start)
        return img_feats
    ...
    def extract_pts_feat(
        ...
        if self.with_backbone:
            if isinstance(self.backbone, (SECONDFuse, SECONDFuseLatency)):
        ...
        self.fusion_meter.update(self.middle_encoder.fusion_meter.val+self.backbone.fusion_meter.val)
        self.pts_feats_meter.update(self.middle_encoder.pts_feats_meter.val+self.backbone.pts_feats_meter.val+neck_time)
