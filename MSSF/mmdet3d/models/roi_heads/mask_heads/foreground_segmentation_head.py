...
class ForegroundSegmentationHead(BaseModule):
    ...
    def get_targets(self, points_bxyz: torch.Tensor,
        ...
        gt_bboxes_3d = []
        gt_labels_3d = []
        # fig bug: batch index is not continuous because we use empty voxel
        seg_targets = points_bxyz.new_zeros(points_bxyz.shape[0]).long()
        batch_mask_list = []
        for idx in range(batch_size):
            coords_idx = points_bxyz[:, 0] == idx
            batch_mask_list.append(coords_idx)
        ...
        seg_targets_list, = multi_apply(self._get_targets_single, points_xyz_list,
                                   gt_bboxes_3d, gt_labels_3d)
        for i in range(batch_size):
            seg_targets[batch_mask_list[i]] = seg_targets_list[i]
