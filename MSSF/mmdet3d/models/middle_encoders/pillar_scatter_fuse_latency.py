...
class PointPillarsScatterFuseLatency(nn.Module):
    ...
    def __init__(self, in_channels: int, output_shape: List[int], voxel_layer, sparse_shape, num_features, out_channels=256, loss_seg_weight=1.0, optional_cfg=dict()):
        super().__init__()
    ...
    def forward(self,
        ...
        if self.use_img:
            ...
            print(f'fusion block in pillar scatter time: {end-start}')
        ...
        self.fusion_meter.update(end-start)
        ...
        if self.use_img:
            if self.no_pillar:
                ...
                batch_canvas = torch.cat([batch_canvas, x], dim=1)

        end = time.perf_counter()
        self.pts_feats_meter.update(end-start)
        return batch_canvas, seg_results_dict
