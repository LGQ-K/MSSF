...
class SparseEncoderVoxelNeXtFPN3D(SparseEncoder):
    ...
    def indice_unique_out_3d(self, x_conv):
        features_cat = x_conv.features # torch.Size([13314, 256])
        indices_cat = x_conv.indices[:, [0, 1, 2, 3]] # shape:torch.Size([15216, 4])
...
#            encode_features.append(x)
        ...
        if debug:
            x_conv5.indices[:, 1:] *= 2
            x_conv6.indices[:, 1:] *= 4
            x_conv5.spatial_shape = x_conv4.spatial_shape
            x_conv6.spatial_shape = x_conv4.spatial_shape
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            # interp_out = laterals[i]
            # interp_out.indices[:, 1:] *= 2
            # laterals[i - 1] = laterals[i - 1].replace_feature(torch.cat([laterals[i - 1].features,interp_out.features]))
            # laterals[i - 1].indices = torch.cat([laterals[i - 1].indices, interp_out.indices])
            # laterals[i - 1] = self.indice_unique_out_3d(laterals[i - 1])

        # build outputs
        out = self.fpn_conv_out(laterals[0])

        out_bev = self.bev_out(out)
        ...
        if debug:
            ...
            vis.fig_save_canvas.print_figure('./middle_vis/out_bev.png')
            vis.set_image(out.dense().sum([0,1]).T.flip(0).flip(1).cpu().numpy())
            vis.fig_save_canvas.print_figure('./middle_vis/out.png')
            vis.set_image(spatial_features.dense().sum([0,1]).T.flip(0).flip(1).cpu().numpy())
            vis.fig_save_canvas.print_figure('./middle_vis/spatial_features.png')
