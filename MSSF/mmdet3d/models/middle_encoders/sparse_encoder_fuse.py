...
class SparseEncoderFuse(nn.Module):
    ...
    def init_query(self, img_feats, x_conv, voxel_data_dict, batch_input_meta):
        ...
        if 'img' in self.query_init_feat_src:
            ...
            for i in range(bs):
                sampled_img_feats[batch_mask_list[i]] = sampled_img_feats_list[i].type_as(sampled_img_feats)
    ...
    def forward(self, voxel_features: Tensor, coors: Tensor,
        ...
        for i, encoder_layer in enumerate(self.encoder_layers):
            loc = f'x_conv{i+1}'
            if i > 0:
                x = encoder_layer[0](x) # spconv or conv_input
                ...
                if i != 0 and j == 0 and block_type == 'conv_module':
                    ...
                    if not self.legacy:
                        if j == len(blocks) - 1 and i != len(
                            ...
                            assert conv_cfg['type'] == 'SubMConv3d'
