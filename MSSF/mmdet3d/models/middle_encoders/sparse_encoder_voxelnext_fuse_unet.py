...
class SparseEncoderVoxelNeXtFuseUNet(nn.Module):
    ...
    def __init__(
        ...
        if not self.use_img:
            ...
            self.seg_cfg = self.optional_cfg.get('seg_cfg', dict(
                ...
                seg_locations=[],
                seg_sup_locations=[],
                seg_apply_location=None
            ))
            self.seg_locations = self.seg_cfg['seg_locations']
            self.use_seg = len(self.seg_locations) > 0
            self.seg_heads = nn.ModuleDict()
            for loc in self.seg_locations:
                selector = 0
                assert self.seg_cfg['seg_method'] == 'final_seg'
        ...
        if self.unet is not None:
            unet_seg_head = dict(type='ForegroundSegmentationHead',
                            in_channels=64)
            self.seg_heads['unet'] = MODELS.build(unet_seg_head)
    ...
    def forward(self, voxel_features: Tensor, coors: Tensor,
        ...
        # x_unet = self.unet(voxel_features, coors, batch_size)
        # xyz = voxel_aggregation_utils.get_voxel_centers(x_unet.indices[:, 1:4], 1, self.meta_dict['voxel_size'],
        #             self.meta_dict['point_cloud_range'])
        # bxyz = torch.cat([x_unet.indices[:, 0].float().unsqueeze(1), xyz], dim=-1)
        # unet_seg_results = self.seg_heads['unet'](x_unet.features)
        # centroids_img = []
        # for i in range(bs):
        #     batch_mask = (bxyz[:,0].int()==i)
        #     xyz_batch = xyz[batch_mask]
        #     proj_mat = get_proj_mat_by_coord_type(batch_input_metas[i], self.coord_type)
        #     proj_mat = torch.tensor(proj_mat[2]).cuda()
        #     batch_input_metas[i].update(img_shape=(1216,1936))
        #     batch_input_metas[i].update(input_shape=(1216,1952))
        #     centroids_img.append(proj_to_img(xyz_batch, batch_input_metas[i], xyz_batch.new_tensor(proj_mat), self.coord_type, normalize=True))
        # centroids_img = torch.cat(centroids_img)
        # centroids_img = torch.cat([bxyz[:,0].unsqueeze(1) ,centroids_img], dim=-1)
        # voxel_dict = {'centroids': bxyz, 'centroids_img': centroids_img}
        # seg_results_dict = dict()
        # seg_results_dict['unet'] = dict(seg_results=unet_seg_results, voxel_dict=voxel_dict)
        # debug=False
        # if debug:
        ...
        for i in range(self.stage_num):
            ...
            if loc in self.fusion_locations:
                ...
                non_empty_centroids = centroids[non_empty_mask]
                voxel_dict = {'centroids': non_empty_centroids, 'inds': voxel_data_dict[loc]['non_empty_inds'], 'xconv_indices':x.indices}
            ...
            if (len(self.fusion_locations) == 0) and (len(self.seg_locations) > 0) and (loc in self.seg_locations):
                xyz = voxel_aggregation_utils.get_voxel_centers(x.indices[:, 1:4], self.meta_dict['multi_scale_3d_strides'][loc], self.meta_dict['voxel_size'],
                            self.meta_dict['point_cloud_range'])
                bxyz = torch.cat([x.indices[:, 0].float().unsqueeze(1), xyz], dim=-1)
                centroids_img = []
                for j in range(bs):
                    batch_mask = (bxyz[:,0].int()==j)
                    xyz_batch = xyz[batch_mask]
                    proj_mat = get_proj_mat_by_coord_type(batch_input_metas[j], self.coord_type)
                    proj_mat = torch.tensor(proj_mat[2]).cuda()
                    batch_input_metas[j].update(img_shape=(1216,1936))
                    batch_input_metas[j].update(input_shape=(1216,1952))
                    centroids_img.append(proj_to_img(xyz_batch, batch_input_metas[j], xyz_batch.new_tensor(proj_mat), self.coord_type, normalize=True))
                centroids_img = torch.cat(centroids_img)
                centroids_img = torch.cat([bxyz[:,0].unsqueeze(1) ,centroids_img], dim=-1)

                seg_results = self.seg_heads[loc](x.features)
                voxel_dict = {'centroids': bxyz, 'centroids_img': centroids_img}
                seg_results_dict[loc] = dict(seg_results=seg_results, voxel_dict=voxel_dict)
            if (len(self.fusion_locations) == 0) and (len(self.seg_locations) > 0) and (loc in self.seg_cfg['seg_apply_location']):
                x.replace_feature(x.features*seg_results_dict[loc]['seg_results']['seg_preds'])
